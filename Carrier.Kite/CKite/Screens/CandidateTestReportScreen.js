...
const CandidateTestReport = ({ route, navigation }) => {
  const { subModuleName, score, maxScore, percentage, questionsAttempted, totalQuestions, timeTaken, totalTime, } = route.params;

  const improvementLink = `https://example.com/improve/${subModuleName.toLowerCase().replace(/\s+/g, '-')}`;

  // Function to autosave data
  const saveReportData = async () => {
    const reportData = { subModuleName, score, maxScore, percentage, questionsAttempted, totalQuestions, timeTaken, totalTime,};
  ...
  useEffect(() => {
    saveReportData();
  }, [subModuleName, score, percentage]);

  return (
    // Wrap the whole content in LinearGradient to apply gradient background
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.mainreport}>
          ...
          <View style={styles.linkContainer}>
            <Text>Click on Below Link</Text>
            <Text
              style={styles.link}
              onPress={() => Linking.openURL(improvementLink)}
            >
              View improvement resources for {subModuleName}
            </Text>
          </View>
